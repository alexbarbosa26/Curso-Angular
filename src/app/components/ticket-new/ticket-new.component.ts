...
export class TicketNewComponent implements OnInit {
  ...
  register() {
    this.message = {};
    this.ticketService.createOrUpdate(this.ticket).subscribe((responserApi: ResponseApi) => {

      this.ticket = new Ticket('',0, '', '','', '', null, null, '', null);
      let ticket: Ticket = responserApi.data;
      this.form.resetForm();
      this.showMessage({
        type: 'success',
        text: `Solicitação registrada com sucesso: ${ticket.number}`
      });
  ...
  onFileChange(event): void {
    if (event.target.files[0].size > 2000000) {
      this.showMessage({
        type: 'error',
        text: 'Imagem deve ter no Máximo 2 MB.'
      })
    } else {
      this.ticket.image = '';
